...
public class FileContentFixer {
    ...
    /**
     * Like {@link #fixFileContent(CharSequence)} but also patches the snapshot specifically for completion,
     * so incomplete constructs around the caret become parseable by javac.
     *
     * Currently supported:
     * - {@code new <caret>} (injects a dummy identifier and parentheses at the caret)
     */
    public CharSequence fixFileContentForCompletion(CharSequence content, int caretOffset) {
        CharSequence fixed = fixFileContent(content);
        ...
        int offset = Math.min(caretOffset, s.length());

        // If caret is right after `new` (possibly with whitespace), inject an identifier at the caret
        // so the parser can build a NewClassTree around the cursor.
        if (isAfterKeywordNew(s, offset)) {
            StringBuilder sb = new StringBuilder(s.length() + INJECTED_IDENT.length() + 2);
            sb.append(s, 0, offset);
            sb.append(INJECTED_IDENT).append("()");
            sb.append(s, offset, s.length());
            return sb;
        }
    ...
    private static boolean isAfterKeywordNew(String text, int offset) {
        if (text == null) {
            return false;
        }
        ...
        return "new".contentEquals(text.subSequence(start, end + 1));
    }
    ...
    private void fixError(Scanner scanner, CharSequence content, List<Insertion> insertions) {
        int errPos = scanner.errPos();
        if (content.charAt(errPos) == '.' && errPos > 0 && content.charAt(errPos - 1) == '.') {
